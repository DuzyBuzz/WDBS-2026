...
public sealed record LedgerReportDocumentData(
	string ReportTitle,
...
private readonly LedgerReportDocumentData _document;
...
public LedgerReportPrintHelper(LedgerReportDocumentData document)
{
_document = document;
}
...
IReadOnlyList<ColumnLayout> columns = BuildColumnLayouts(bounds);
	float tableTop = bounds.Top + TotalHeaderHeight;
...
graphics.DrawString(_document.PeriodCaption, captionFont, textBrush, new RectangleF(centerLeft, bounds.Top + 33F, centerWidth, 12F), format);
	...
	DrawConcessionaireHeader(graphics, bounds);
    }

private void DrawConcessionaireHeader(Graphics graphics, RectangleF bounds)
{
	float subHeaderTop = bounds.Top + HeaderHeight;
	using var subHeaderBrush = new SolidBrush(Color.FromArgb(235, 242, 250));
	using var subHeaderPen = new Pen(AppTheme.BorderColor);
	using var subHeaderTextBrush = new SolidBrush(AppTheme.BodyTextColor);
	using var subHeaderBoldFont = new Font("Segoe UI Semibold", 8.5F, FontStyle.Bold);
	using var subHeaderCaptionFont = new Font("Segoe UI", 8F, FontStyle.Regular);
	using var leftFormat = new StringFormat { Alignment = StringAlignment.Near, LineAlignment = StringAlignment.Center };

	graphics.FillRectangle(subHeaderBrush, bounds.Left, subHeaderTop, bounds.Width, SubHeaderHeight);
	graphics.DrawRectangle(subHeaderPen, bounds.Left, subHeaderTop, bounds.Width, SubHeaderHeight);

	float textLeft = bounds.Left + CellPadding * 2F;
	float textWidth = bounds.Width - (CellPadding * 4F);
	float lineHeight = (SubHeaderHeight / 2F) - CellPadding;

	string accountLabel = string.IsNullOrWhiteSpace(_document.ConcessionaireCode)
		? "Account: N/A"
		: $"Account: {_document.ConcessionaireCode}";
	string customerLabel = string.IsNullOrWhiteSpace(_document.ConcessionaireName)
		? _document.PeriodCaption
		: _document.ConcessionaireName;
	if (!string.IsNullOrWhiteSpace(_document.ConcessionaireAddress))
	{
		customerLabel = $"{customerLabel} | Address: {_document.ConcessionaireAddress}";
	}

	graphics.DrawString(accountLabel, subHeaderBoldFont, subHeaderTextBrush,
		new RectangleF(textLeft, subHeaderTop + 2F, textWidth, lineHeight), leftFormat);
	graphics.DrawString($"Customer: {customerLabel}", subHeaderCaptionFont, subHeaderTextBrush,
		new RectangleF(textLeft, subHeaderTop + lineHeight, textWidth, lineHeight), leftFormat);
}
...
var bounds = new RectangleF(18F, 18F, pageHeight - 36F, pageWidth - 36F);
...
DrawCenteredPdfText(gfx, _document.PeriodCaption, captionFont, textBrush, centerLeft, bounds.Top + 33F, centerWidth);
	...
	DrawPdfConcessionaireHeader(gfx, bounds);
...
private void DrawPdfConcessionaireHeader(XGraphics gfx, RectangleF bounds)
{
	float subHeaderTop = bounds.Top + HeaderHeight;
	var subHeaderFillBrush = new XSolidBrush(ToXColor(Color.FromArgb(235, 242, 250)));
	var subHeaderBorderPen = new XPen(ToXColor(AppTheme.BorderColor), 0.5);
	var subHeaderBoldFont = new XFont("Arial", 9, XFontStyleEx.Bold);
	var subHeaderCaptionFont = new XFont("Arial", 8, XFontStyleEx.Regular);
	var subHeaderTextBrush = new XSolidBrush(ToXColor(AppTheme.BodyTextColor));
	float textLeft = bounds.Left + CellPadding * 2F;
	float textWidth = bounds.Width - (CellPadding * 4F);
	float lineHeight = (SubHeaderHeight / 2F) - CellPadding;
	string accountLabel = string.IsNullOrWhiteSpace(_document.ConcessionaireCode)
	...
	gfx.DrawRectangle(subHeaderFillBrush, bounds.Left, subHeaderTop, bounds.Width, SubHeaderHeight);
	gfx.DrawRectangle(subHeaderBorderPen, bounds.Left, subHeaderTop, bounds.Width, SubHeaderHeight);
	DrawPdfText(gfx, accountLabel, subHeaderBoldFont, subHeaderTextBrush,
		new XRect(textLeft, subHeaderTop + 2F, textWidth, lineHeight), XStringAlignment.Near, wrap: false);
	DrawPdfText(gfx, $"Customer: {customerLabel}", subHeaderCaptionFont, subHeaderTextBrush,
		new XRect(textLeft, subHeaderTop + lineHeight, textWidth, lineHeight), XStringAlignment.Near, wrap: false);
}
...
IXLWorksheet sheet = workbook.Worksheets.Add("Ledger Report");

int totalColumns = Math.Max(1, definitions.Length);
...
sheet.Cell(4, 1).Value = string.IsNullOrWhiteSpace(_document.ConcessionaireCode)
	? "Account: N/A"
	: $"Account: {_document.ConcessionaireCode}";
sheet.Cell(5, 1).Value = string.IsNullOrWhiteSpace(_document.ConcessionaireName)
	? "Customer: N/A"
	: $"Customer: {_document.ConcessionaireName}";
sheet.Cell(6, 1).Value = string.IsNullOrWhiteSpace(_document.ConcessionaireAddress)
	? string.Empty
	: $"Address: {_document.ConcessionaireAddress}";
sheet.Cell(7, 1).Value = $"Printed: {_document.PrintedAt:MMMM dd, yyyy hh:mm tt} - {_document.PrintedBy}";

foreach (int row in new[] { 1, 2, 3, 4, 5, 6, 7 })
{
...
{
sheet.Cell(9, columnIndex + 1).Value = definitions[columnIndex].HeaderText;
}
...
SetExcelCellValue(sheet.Cell(rowIndex + 10, columnIndex + 1), value);
}
...
{
int lastRow = Math.Max(9, totalRows + 9);
var tableRange = sheet.Range(9, 1, lastRow, definitions.Length);
var table = tableRange.CreateTable("LedgerReportTable");
...
{
int totalRows = _document.Rows.Rows.Count;
	float maxRowsHeightPerPage = contentBounds.Height - TotalHeaderHeight - TableHeaderHeight - FooterHeight - 2F;
...
resolvedHeight = Math.Max(resolvedHeight, measuredHeight + (CellPadding * 2F) + WrappedRowAllowance);
}
...
return
[
			new("transaction_date", "Date",           10F, StringAlignment.Center),
			new("reference_no",    "Reference No.",  10F, StringAlignment.Center),
			new("remarks",         "Remarks",        15F, StringAlignment.Near, true),
			new("debit",           "Debit",           9F, StringAlignment.Far),
			new("credit",          "Credit",          9F, StringAlignment.Far),
			new("running_balance", "Balance",         9F, StringAlignment.Far),
			new("water_charge",    "Water Charge",    9F, StringAlignment.Far),
			new("tax_amount",      "Tax",             6F, StringAlignment.Far),
			new("arrears_amount",  "Arrears",         7F, StringAlignment.Far),
			new("penalty_amount",  "Penalty",         7F, StringAlignment.Far),
			new("scf_amount",      "SCF",             6F, StringAlignment.Far)
];
}
...
if (string.Equals(columnName, "transaction_date", StringComparison.OrdinalIgnoreCase))
{
...
{
if (string.Equals(columnName, "running_balance", StringComparison.OrdinalIgnoreCase) && amount < 0m)
{
amount = 0m;
}
